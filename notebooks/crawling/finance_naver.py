...
def get_exchange_rate_data(selected):
    ...
    total_rate_data_view(df_selected)


def total_rate_data_view(df_selected):
    # 원하는 열만 선택
...
currency_code = int(input('통화유형을 선택해주세요(1:USD, 2:EUR, 3:JPY): '))
currency_type = ['USD', 'EUR', 'JPY']
currency_name = ['미국 달러', '유럽연합 유로', '일본 엔(100)']
currency_symbols = ['$','€', '¥']
selected = currency_type[currency_code-1]
df_selected = get_exchange_rate_data(selected)
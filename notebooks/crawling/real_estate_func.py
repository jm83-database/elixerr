...
def draw_chart(df_rate):
    # 차트 그리기
    df_rate.head(30).plot(x = '등록일', y = ['전국', '서울', '수도권'], figsize = (15,6), title = '2019.07~2020.01 부동산 가격 추이')
    plt.show()
    ...
    montly_analysis.plot(figsize = (15,6), title = '월별 부동산 가격 추이(평균)')
    plt.show()

    # 년도별 추이 분석(평균)
    yearly_analysis = df_rate.groupby(df_rate.등록일.dt.year)[['전국', '서울', '수도권']].mean()
    yearly_analysis.plot(figsize = (15,6), title = '년도별 부동산 가격 추이(평균)')
    plt.show()
    
df = load_data()
df_rate = clean_data(df)
print(draw_chart(df_rate))